...
$parentModule = 'Roles'
if (-not $ModuleName) { $ModuleName = 'Roles.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\Roles'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\Roles.psd1"
$prereqHash = @{
